...
@Tag(name = "Owner Item", description = "점주 상품 관리 API")
@RestController
...
public class OwnerItemController {
    ...
    @Operation(summary = "[점주] 상점별 상품 목록 조회", description = "특정 상점의 모든 상품을 조회합니다.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<CommonResponse<List<ItemResponse>>> getItems(
        ...
        List<ItemResponse> response = itemService.getItemsForOwner(storeId);
        return ResponseEntity.ok(CommonResponse.success(response));
    }

    @Operation(summary = "[점주] 상품 단건 조회", description = "상품 ID로 상품의 상세 정보를 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공"),
    ...
    public ResponseEntity<CommonResponse<ItemResponse>> getItem(
        ...
        ItemResponse response = itemService.getItemForOwner(itemId);
        return ResponseEntity.ok(CommonResponse.success(response));
    }
