...
@Tag(name = "Student Review", description = "학생 리뷰 API")
@RestController
...
public class StudentReviewController {
    ...
    @Operation(summary = "[학생] 리뷰 작성", description = "상점에 리뷰를 작성합니다. (학생만 가능, 상점당 1개)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "리뷰 작성 성공"),
            @ApiResponse(responseCode = "400", description = "잘못된 요청 데이터 (평점 누락 등)", content = @Content(schema = @Schema(implementation = SwaggerErrorResponse.class))),
    ...
    public ResponseEntity<CommonResponse<Long>> createReview(
        ...
        Long reviewId = reviewService.createReviewForStudent(principalDetails.getUser(), storeId, request);
        return ResponseEntity.status(HttpStatus.CREATED).body(CommonResponse.success(reviewId));
    }

    @Operation(summary = "[학생] 리뷰 답글 작성", description = "리뷰에 답글을 작성합니다.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<CommonResponse<Long>> createReply(
        ...
        Long reviewId = reviewService.createReplyForStudent(principalDetails.getUser(), storeId, parentReviewId, request);
        return ResponseEntity.status(HttpStatus.CREATED).body(CommonResponse.success(reviewId));
    }

    @Operation(summary = "[학생] 리뷰 수정", description = "작성한 리뷰 또는 답글을 수정합니다. (본인만 가능)")
    @ApiResponses(value = {
    ...
    @Operation(summary = "[학생] 리뷰 삭제", description = "작성한 리뷰 또는 답글을 삭제합니다. (본인만 가능)")
    @ApiResponses(value = {
    ...
    @Operation(summary = "[학생] 상점 리뷰 목록 조회", description = "특정 상점의 리뷰 목록을 페이징하여 조회합니다.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "[학생] 내 리뷰 목록 조회", description = "내가 작성한 리뷰 목록을 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공")
    })
    ...
    @Operation(summary = "[학생] 상점 리뷰 통계 조회", description = "상점의 평점 평균, 총 리뷰 수, 별점별 개수 분포를 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공"),
            @ApiResponse(responseCode = "404", description = "상점 없음", content = @Content(schema = @Schema(implementation = SwaggerErrorResponse.class)))
    })
    ...
    @Operation(summary = "[학생] 리뷰 신고", description = "특정 리뷰를 신고합니다.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "[학생] 리뷰 좋아요", description = "리뷰에 좋아요를 누릅니다.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "[학생] 리뷰 좋아요 취소", description = "리뷰 좋아요를 취소합니다.")
    @ApiResponses(value = {
